...
* @微信: 252810631
* @版权所有: 请尊重劳动成果
* FileStoreTable 上的抽象层，用来读写InternalRow(数据)
*/
abstract class AbstractFileStoreTable implements FileStoreTable {
    ...
    * @微信: 252810631
    * @版权所有: 请尊重劳动成果
    * 创建一个行键提取器（RowKeyExtractor）。
    * 根据不同的分桶模式（bucket mode），返回不同的行键提取器实例。
    */
    public RowKeyExtractor createRowKeyExtractor() {
        // 根据当前的分桶模式（bucketMode）来决定使用哪种行键提取器
        switch (bucketMode()) {
            // 固定哈希分桶模式
            case HASH_FIXED:
                // 返回固定分桶行键提取器，传入当前的schema（模式）
                return new FixedBucketRowKeyExtractor(schema());
            case HASH_DYNAMIC:// 动态哈希分桶模式
            case CROSS_PARTITION:// 跨分区模式
                // 对于动态哈希分桶模式和跨分区模式，返回动态分桶行键提取器，传入当前的schema
                return new DynamicBucketRowKeyExtractor(schema());
            case BUCKET_UNAWARE:// 无感知分桶模式
                // 返回无感知分桶行键提取器，传入当前的schema
                return new UnawareBucketRowKeyExtractor(schema());
            default:
                // 如果当前分桶模式不是上述任何一种，则抛出不支持的操作异常
