...
public abstract class AbstractCatalog implements Catalog {
    ...
    */
    ...
    public void createTable(Identifier identifier, Schema schema, boolean ignoreIfExists)
        ...
        // 检查当前要创建的表是否不是系统表，因为系统表通常由系统自动管理，不允许用户手动创建
        checkNotSystemTable(identifier, "createTable");
        // 验证标识符名称是否不区分大小写，这通常是为了确保数据库和表的命名一致性
        validateIdentifierNameCaseInsensitive(identifier);
        // 验证模式（Schema）中的字段名称是否不区分大小写，这有助于避免在查询和处理数据时因大小写不一致而导致的错误
        validateFieldNameCaseInsensitive(schema.rowType().getFieldNames());
        validateAutoCreateClose(schema.options());

        // 检查指定的数据库是否存在
        if (!databaseExists(identifier.getDatabaseName())) {
            // 如果数据库不存在，则抛出DatabaseNotExistException异常
            throw new DatabaseNotExistException(identifier.getDatabaseName());
        }
        // 检查指定的表是否已经存在
        if (tableExists(identifier)) {
            // 如果表已经存在，并且ignoreIfExists为true，则忽略创建操作，直接返回
            if (ignoreIfExists) {
                return;
            }
            throw new TableAlreadyExistException(identifier);
        }
        // 复制表的默认选项到模式（Schema）的选项中
